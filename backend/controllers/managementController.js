...
const getDashboardData = async (req, res) => {
  ...
  try {
    ...
    if (role === "pharmacist") {
      ...
      quickStats = {
        "Pending Prescriptions": result[0].count,
        "Today's Validations": 34,
      };
    } else if (role === "branchManager") {
      const [result] = await db.query(
        "SELECT SUM(total_amount) as total FROM `order` WHERE status = 'completed' AND DATE(order_date) = CURDATE()"
      );
      quickStats = {
        "Today's Revenue": result[0].total || 0,
        "Low Stock Items": 8,
      };
    }
